  ...
  try {
    ...
    const buyerLines = await query<any>(
      `SELECT oi.product_slug, oi.name, oi.size, oi.color, oi.quantity, oi.unit_price, oi.line_total,
       ...
       WHERE o.deleted_at IS NULL AND o.status IN ('completed','delivered') AND o.created_at BETWEEN ? AND ?`,
      [start, end]
    );
    ...
    const resellerLines = await query<any>(
      `SELECT roi.product_slug, roi.name, roi.sku, roi.variant_summary, roi.quantity, roi.selling_price AS unit_price, roi.line_total,
       ...
       WHERE ro.deleted_at IS NULL AND ro.status IN ('completed','delivered') AND ro.created_at BETWEEN ? AND ?`,
      [start, end]
    );
    ...
    const posLines = await query<any>(
      `SELECT psi.product_slug, psi.name, psi.sku, psi.size, psi.color, psi.quantity, psi.unit_price, psi.line_total,
       ...
       WHERE s.deleted_at IS NULL AND s.created_at BETWEEN ? AND ? AND s.status = 'completed' AND COALESCE(s.delivery_status, '') <> 'cancelled'`,
      [start, end]
    );
    ...
    const buyerOrders = await query<{ id: number; total: string }>(
      `SELECT id, total FROM orders WHERE deleted_at IS NULL AND status IN ('completed','delivered') AND created_at BETWEEN ? AND ?`,
      [start, end]
    );
    const resellerOrders = await query<{ id: number; amount: string }>(
      `SELECT id, amount FROM reseller_orders WHERE deleted_at IS NULL AND status IN ('completed','delivered') AND created_at BETWEEN ? AND ?`,
      [start, end]
    );
    const posSales = await query<{ id: number; total: string }>(
      `SELECT id, total FROM pos_sales WHERE deleted_at IS NULL AND created_at BETWEEN ? AND ? AND status = 'completed' AND COALESCE(delivery_status, '') <> 'cancelled'`,
      [start, end]
    );
