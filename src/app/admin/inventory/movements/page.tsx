...
export default function StockMovementsPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      const params = new URLSearchParams({ search, direction, type, from, to });
  ...
  }, [search, direction, type, from, to]);

  const downloadPdf = () => {
    const params = new URLSearchParams({ search, direction, type, from, to, format: "pdf" });
    window.location.href = `/api/admin/inventory/movements?${params}`;
  };
  ...
  return <div>
    ...
    <div className="mt-6 flex flex-col flex-wrap gap-3 rounded-2xl border border-navy-800/10 bg-white p-5 sm:flex-row sm:items-end">
      ...
      <select value={type} onChange={(event) => setType(event.target.value)} className="input sm:max-w-[180px]"><option value="all">All movement types</option><option value="stock_in">Stock in</option><option value="stock_out">Stock out</option><option value="adjustment">Adjustments</option><option value="pos_sale">POS sales</option><option value="reseller_order">Reseller orders</option><option value="system">Other system movements</option></select>
      <label className="text-xs font-semibold text-navy-800/55">From<input type="date" value={from} max={to || undefined} onChange={(event) => setFrom(event.target.value)} className="input mt-1 sm:w-[155px]" /></label>
      <label className="text-xs font-semibold text-navy-800/55">To<input type="date" value={to} min={from || undefined} onChange={(event) => setTo(event.target.value)} className="input mt-1 sm:w-[155px]" /></label>
      <button type="button" onClick={downloadPdf} className="rounded-lg bg-navy-800 px-4 py-2.5 text-sm font-semibold text-white hover:bg-navy-700">Download PDF</button>
    </div>
